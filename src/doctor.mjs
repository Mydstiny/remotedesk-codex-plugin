...
export async function doctor({ probe = false, command, signal } = {}) {
  ...
  try {
    ...
    const { stdout } = await exec(selected.command, [...selected.prefix, '--version'], {
      timeout: 5000,
      maxBuffer: 4096,
      windowsHide: true,
      signal,
    ...
    if (probe) {
      ...
      rpc = new StdioRpc(
        selected.command,
        [...selected.prefix, '--disable', 'plugins', '--disable', 'hooks', 'app-server'],
        { cwd: directory },
      );
      ...
      const initialized = await rpc.request('initialize', {
        clientInfo: {
          name: 'remotedesk_ai0_probe',
          title: 'RemoteDesk AI0 Probe',
          version: compatibility.adapterVersion,
        },
        capabilities: { experimentalApi: true },
      });
      if (typeof initialized?.userAgent !== 'string')
        throw new BridgeError('INITIALIZE_SHAPE_CHANGED');
      ...
      if (
        ...
        started.approvalPolicy !== 'untrusted' ||
      ...
      report.warnings.push(
        'NO_MODEL_TURN_APPROVAL_CANCEL_OR_RESUME_TESTED',
        'ENGINE_MAY_WRITE_OWN_OPERATIONAL_LOGS',
      );
    }
    report.status = 'ok';
  } catch (error) {
    ...
    if (rpc) {
      try {
        ...
        check('cleanup', 'fail', 'PROCESS_CLEANUP_UNCONFIRMED');
      }
    }
