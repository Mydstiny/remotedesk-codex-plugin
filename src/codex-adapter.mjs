...
export function remoteConfig(profile,mcpNames=[],inheritedEnvironment=[],libraries=[]) {
 ...
 return { default_permissions:profile, permissions:{[profile]:{filesystem:{...Object.fromEntries(libraries.map(p=>[p,'read'])),':root':'deny',':minimal':'read',':tmpdir':'deny',':slash_tmp':'deny',':workspace_roots':{'.':'read','.git':'read','.codex':'read','.agents':'read'}},network:{enabled:false}}},
  ...
  notify:[],mcp_servers:Object.fromEntries(mcpNames.map(n=>[n,{enabled:false}])),allow_login_shell:false,shell_environment_policy:{inherit:'none',ignore_default_excludes:false,set:environment},web_search:'disabled',approval_policy:APPROVAL_POLICY,approvals_reviewer:'user' };
}
...
export class CodexAdapter {
 ...
 constructor({command,providerOverrides,executor}={}) {this.customExecutor=executor;this.explicit=command;this.providerOverrides=providerOverrides;this.rpcs=new Map();this.turns=new Map();this.diffs=new Map();this.loading=new Map();this.aborters=new Map();this.runs=new Map();this.closed=false;}
 ...
 async open(s,p) {
  ...
  const rpc=new StdioRpc(command.command,[...command.prefix,...DISABLED_FEATURES.flatMap(f=>['--disable',f]),'app-server'],{cwd:p.path,timeoutMs:45000,maxFrameBytes:16000000,requestHandler:(method,params)=>this.serverRequest(s,method,params)});
  try {
   ...
   const params={cwd:p.path,approvalPolicy:APPROVAL_POLICY,approvalsReviewer:'user',config,...(p.provider?{modelProvider:p.provider}:{}),...(p.model?{model:p.model}:{})};
   ...
   requireThat(!this.closed,'ADAPTER_DISPOSED');rpc.on('notification',message=>this.notification(s,message));this.rpcs.set(s.id,rpc);
   return rpc;
  } catch(e) {await rpc.close();throw e;}
 }
 async create(s,p){await this.connect(s,p);return {upstream:s.upstream};}
 ...
 async read(s,{cursor}={}) {
  ...
  return {status:this.turns.has(s.id)?'running':'idle',turns:turns.data??turns.turns??[],nextCursor:turns.nextCursor??null};
 }
 ...
 async deactivate(s){requireThat(!this.runs.has(s.id),'TURN_ALREADY_RUNNING');const rpc=this.rpcs.get(s.id);if(rpc){await rpc.close();this.rpcs.delete(s.id);}}
 async close(){if(this.closed)return;this.closed=true;for(const run of this.runs.values())run.cancelRequested=true;for(const controller of this.aborters.values())controller.abort();await Promise.allSettled([...this.loading.values()]);const results=await Promise.allSettled([...this.rpcs.values()].map(r=>r.close()));this.rpcs.clear();if(results.some(r=>r.status==='rejected'))throw new Fault('ENGINE_CLEANUP_UNCONFIRMED');await this.executor?.recover();}
}
