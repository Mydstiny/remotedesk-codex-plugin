...
import {
  ...
  projectDiff,
} from '../packages/bridge-core/lib/workspace-tools.mjs';
...
export function remoteConfig(profile, mcpNames = [], inheritedEnvironment = [], libraries = []) {
  ...
  const valueFor = (key) => {
    const name = Object.keys(process.env).find((n) => n.toUpperCase() === key.toUpperCase());
    return allowed.some((n) => n.toUpperCase() === key.toUpperCase())
      ? (process.env[name] ?? '')
      : '';
  };
  const environment = Object.fromEntries(
    [...new Set([...inheritedEnvironment, ...allowed])].map((key) => [key, valueFor(key)]),
  );
  return {
    ...
    permissions: {
      [profile]: {
        filesystem: {
          ...Object.fromEntries(libraries.map((p) => [p, 'read'])),
          ':root': 'deny',
          ':minimal': 'read',
          ':tmpdir': 'deny',
          ':slash_tmp': 'deny',
          ':workspace_roots': { '.': 'read', '.git': 'read', '.codex': 'read', '.agents': 'read' },
        },
    ...
    mcp_servers: Object.fromEntries(mcpNames.map((n) => [n, { enabled: false }])),
    allow_login_shell: false,
    shell_environment_policy: { inherit: 'none', ignore_default_excludes: false, set: environment },
    web_search: 'disabled',
  ...
  for (const p of (process.env.PATH ?? '').split(delimiter)) {
    try {
      if (process.platform === 'win32') {
        const file = join(p, 'node_modules', '@openai', 'codex', 'bin', 'codex.js');
        await access(file);
...
export class CodexAdapter {
  ...
  constructor({ command, providerOverrides, executor, diagnostic } = {}) {
  ...
  async connect(s, p) {
    ...
    if (this.rpcs.size + this.loading.size >= 8) {
      const idle = [...this.rpcs.keys()].find((id) => !this.runs.has(id));
      if (idle) await this.deactivate({ id: idle });
    }
    requireThat(this.rpcs.size + this.loading.size < 8, 'ENGINE_SESSION_LIMIT');
    const pending = this.open(s, p);
  ...
  async open(s, p) {
    requireThat(!s.upstream || s.executionProfile === 'container-v1', 'SESSION_PROFILE_UNVERIFIED');
    const command = await codexCommand(this.explicit);
    const { stdout } = await exec(command.command, [...command.prefix, '--version'], {
      timeout: 5000,
      maxBuffer: 4096,
      windowsHide: true,
    });
    ...
    const spawn = (extra) => {
      const rpc = new StdioRpc(
        ...
        ],
        {
          ...
          requestHandler: (method, params) => this.serverRequest(s, method, params),
        },
      );
    ...
    try {
      await rpc.request('initialize', {
        clientInfo: { name: 'remotedesk_bridge', title: 'RemoteDesk', version: '0.2.0' },
      ...
      const provider =
        ...
        'openai';
      let selected, cursorModel;
      let pageCount = 0;
      do {
        const page = await rpc.request('model/list', {
          limit: 100,
          ...(cursorModel ? { cursor: cursorModel } : {}),
        });
        selected = page.data.find((m) => (model ? m.model === model : m.isDefault));
        cursorModel = page.nextCursor;
        requireThat(++pageCount <= 10, 'MODEL_CATALOG_LIMIT');
      } while (!selected && cursorModel);
      model ??= selected?.model;
      string(model, 200);
      string(provider, 200);
      await rpc.close();
      metadataDirectory = await mkdtemp(join(tmpdir(), 'remotedesk-codex-metadata-'));
      await privateDirectory(metadataDirectory, { create: true });
      ...
      const catalog = join(metadataDirectory, 'models.json');
      await writeFile(
        catalog,
        JSON.stringify(modelProfile(model, { ...selected, inputModalities })),
        { mode: 0o600 },
      );
      ...
      const config = {
        ...remoteConfig(
          profile,
          names,
          Object.keys(effective.config?.shell_environment_policy?.set ?? {}),
        ),
      ...
      const result = await rpc.request(
        ...
        s.upstream
          ...
          : {
              ...params,
              environments: [],
              dynamicTools: workspaceTools.map((tool) => ({ type: 'function', ...tool })),
            },
      );
      requireThat(
        result.activePermissionProfile?.id === profile &&
          ...
          Object.entries(APPROVAL_POLICY.granular).every(
            ([key, value]) => result.approvalPolicy?.granular?.[key] === value,
          ),
        'EXECUTION_PROFILE_MISMATCH',
      );
      s.upstream = result.thread.id;
      s.model = model;
      s.provider = provider;
      s.executionProfile = 'container-v1';
      ...
      requireThat(!this.closed, 'ADAPTER_DISPOSED');
      rpc.on('notification', (message) => this.notification(s, message));
      this.rpcs.set(s.id, rpc);
      this.metadata.set(s.id, metadataDirectory);
      return rpc;
    } catch (e) {
      await rpc.close();
  ...
  async resume(s) {
    await this.connect(s, this.project(s));
  }
  async read(s, { cursor } = {}) {
  ...
  async start(s, text, attachments = []) {
    ...
    this.turns.set(s.id, 'starting');
    this.aborters.set(s.id, run.controller);
    try {
      const rpc = await this.connect(s, this.project(s));
      requireThat(!run.cancelRequested, 'TURN_CANCELLED_BEFORE_DISPATCH');
      requireThat(
        !attachments.some((a) => a.mime !== 'text/plain') || s.inputModalities?.includes('image'),
      ...
      run.dispatched = true;
      const r = await rpc.request('turn/start', {
        threadId: s.upstream,
        input,
        environments: [],
        clientUserMessageId: randomUUID(),
      });
      run.turnId ??= r.turn.id;
      run.readyResolve();
  ...
  async interrupt(s, run) {
    ...
    run.interruptPromise ??= this.rpcs
      .get(s.id)
      .request('turn/interrupt', { threadId: s.upstream, turnId: run.turnId });
      ...
      try {
        await Promise.race([
          ...
          new Promise((_, reject) => {
            timer = setTimeout(() => reject(new Fault('CANCEL_DID_NOT_SETTLE')), 15000);
          }),
        ]);
      } finally {
        clearTimeout(timer);
  ...
  notification(s, { method, params }) {
    ...
    if (/^(turn\/|item\/|thread\/tokenUsage\/)/.test(method))
  ...
  async serverRequest(s, method, params) {
    requireThat(
      params?.threadId === s.upstream && params.turnId === this.turns.get(s.id),
    ...
    try {
      ...
      return {
        contentItems: [
          { type: 'inputText', text: e instanceof Fault ? e.code : 'TOOL_EXECUTION_FAILED' },
        ],
        success: false,
      };
    }
  ...
  async deactivate(s) {
    ...
    const directory = this.metadata.get(s.id);
    if (directory) {
      await rm(directory, { recursive: true, force: true });
      this.metadata.delete(s.id);
    }
  }
  async close() {
    ...
    const results = await Promise.allSettled([...this.rpcs.values()].map((r) => r.close()));
    this.rpcs.clear();
