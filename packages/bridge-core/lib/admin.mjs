...
export async function init(
  directory,
  { engine, hosts = ['localhost', '127.0.0.1'], host = '127.0.0.1', port = 9443 } = {},
) {
  ...
  const config = {
    ...
    host,
    port,
    coordinationDirectory: join(homedir(), '.remotedesk', 'coordination'),
    projects: [],
  };
  ...
  store.put('meta', 'instance', { id: token(), created: Date.now() });
  store.close();
  return {
    initialized: true,
    caFile: join(resolve(directory), 'pki', 'ca.pem'),
    url: `https://${host.includes(':') ? '[' + host + ']' : host}:${port}`,
  };
}
export async function configuration(directory) {
  return JSON.parse(await readFile(join(directory, 'config.json'), 'utf8'));
}
export async function addProject(directory, input) {
  fields(input, ['id', 'path', 'title', 'provider', 'model', 'image', 'vision'], ['id', 'path']);
  identifier(input.id);
  const path = await realpath(input.path);
  requireThat((await stat(path)).isDirectory());
  const state = await realpath(directory);
  requireThat(!inside(path, state) && !inside(state, path), 'STATE_PROJECT_OVERLAP');
  const c = await configuration(directory);
  requireThat(
    !c.projects.some((p) => p.id === input.id) && c.projects.length < 32,
    'PROJECT_EXISTS_OR_LIMIT',
  );
  const project = {
    id: input.id,
    path,
  ...
  if (input.vision !== undefined) {
    requireThat(typeof input.vision === 'boolean');
    project.vision = input.vision;
  }
  if (input.image)
    project.image = string(input.image, 300, /^(?:[a-zA-Z0-9./:_-]+@)?sha256:[a-f0-9]{64}$/);
  c.projects.push(project);
  await writeFile(join(directory, 'config.json'), JSON.stringify(c, null, 2) + '\n', {
    mode: 0o600,
  });
...
export async function invite(directory, { projects, role = 'operator' }) {
  const c = await configuration(directory);
  requireThat(['viewer', 'operator'].includes(role));
  requireThat(
  ...
  try {
    ...
    return {
      code,
      expires,
      ca: await readFile(join(directory, 'pki', 'ca.pem'), 'utf8'),
      serverInstance: store.get('meta', 'instance').id,
    };
  } finally {
    store.close();
  }
...
export function revoke(directory, deviceId) {
  ...
  try {
    ...
    requireThat(d, 'DEVICE_NOT_FOUND');
    d.revoked = true;
    d.generation++;
    store.put('device', d.id, d);
    return { revoked: d.id };
  } finally {
    store.close();
  }
...
export function status(directory) {
  ...
  try {
    return {
      ...
      operations: store
        .all('operation')
        .reduce((a, o) => ((a[o.status] = (a[o.status] ?? 0) + 1), a), {}),
    };
  } finally {
    store.close();
  }
