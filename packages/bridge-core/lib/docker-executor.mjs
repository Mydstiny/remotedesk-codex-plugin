...
export class DockerExecutor {
  constructor(store, { command = 'docker' } = {}) {
    this.store = store;
    this.command = command;
    this.owner = store.get('meta', 'instance').id;
  }
  async docker(args, options = {}) {
    return exec(this.command, args, {
      timeout: 15000,
      maxBuffer: 1048576,
      windowsHide: true,
      ...options,
  ...
  async check(project) {
    ...
    const context = JSON.parse(
      (await this.docker(['context', 'inspect', '--format', '{{json .Endpoints.docker.Host}}']))
        .stdout,
    );
    requireThat(
      typeof context === 'string' && /^(unix:\/\/|npipe:\/\/)/.test(context),
      'DOCKER_LOCAL_DAEMON_REQUIRED',
    );
    ...
    requireThat(
      (await this.docker(['info', '--format', '{{.OSType}}'])).stdout.trim() === 'linux',
    ...
    if (exists) {
      const label = (
        await this.docker([
          ...
          name,
          '--format',
          '{{index .Config.Labels "org.remotedesk.owner"}}',
        ])
      ).stdout.trim();
  ...
  async run(project, command, { signal, readOnly = false } = {}) {
    ...
    const args = [
      ...
      name,
      '--label',
      `org.remotedesk.owner=${this.owner}`,
      '--network',
      'none',
      '--read-only',
      '--cap-drop',
      ...
      user,
      '--mount',
      `type=bind,source=${project.path},target=/workspace${readOnly ? ',readonly' : ''}`,
      '--tmpfs',
    ...
    try {
      requireThat(
        !project.path.includes(',') && !project.path.includes('\n'),
        'DOCKER_PATH_UNSUPPORTED',
      );
      await this.docker(args, { signal });
      let output;
      try {
        output = await this.docker(['start', '--attach', name], {
          timeout: 180000,
          signal,
          maxBuffer: 1048576,
        });
      } catch (e) {
        if (signal?.aborted) throw new Fault('EXECUTION_CANCELLED');
        if (e.killed || e.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER')
          throw new Fault('EXECUTION_LIMIT');
        if (Number.isInteger(e.code)) output = { stdout: e.stdout ?? '', stderr: e.stderr ?? '' };
        else throw new Fault('CONTAINER_EXECUTION_FAILED');
      }
      const status = JSON.parse(
        (await this.docker(['inspect', name, '--format', '{{json .State}}'])).stdout,
      );
      requireThat(
        status.Status === 'exited' && Number.isInteger(status.ExitCode),
        'CONTAINER_DID_NOT_EXIT',
      );
