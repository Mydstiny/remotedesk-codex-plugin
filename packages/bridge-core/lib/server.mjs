...
export class Bridge {
  ...
  async startInternal() {
    ...
    try {
      ...
      this.store.recover();
      for (const lease of this.store.all("lease"))
        this.store.delete("lease", lease.id);
      this.runtime = token();
      ...
      this.adapter.bind({
        ...
        checkpoint: (s, patch) => {
          ...
          this.store.put("session", s.id, {
            ...this.store.get("session", s.id),
            ...s,
          });
        },
        claimActivity: (s, authorize, claim) =>
          this.serializeSession(s.id, async () => {
            requireThat(!this.stopping, "SERVICE_STOPPING");
            authorize();
            ...
            const acquired = await this.projectLocks.acquire(project, s.id);
            try {
              authorize();
              return claim();
            } catch (error) {
              if (acquired && !this.store.get("nativeActivity", s.id))
      ...
      for (const p of this.config.projects) {
        ...
        requireThat(
          !inside(p.path, await realpath(this.directory)),
  ...
  device(req) {
    ...
    requireThat(
      d && !d.revoked && d.expires > Date.now(),
      "DEVICE_UNAUTHORIZED",
    );
    return d;
  }
  ...
  async route(req, res) {
    ...
    try {
      ...
      requireThat(
        (this.deviceInflight.get(d.id) ?? 0) < 8,
      ...
      requireThat(
        req.method === "POST" && req.url === "/v1/rpc",
        "ROUTE_NOT_FOUND",
      );
      ...
      fields(
        call,
        ["method", "params", "operationId", "epoch"],
        ["method", "params"],
      );
      requireThat(
        READ.has(call.method) || MUTATE.has(call.method),
        "METHOD_UNSUPPORTED",
      );
      ...
      try {
        ...
        op.error =
          e instanceof Fault ? e.code : "UPSTREAM_OUTCOME_UNKNOWN_RECONCILE";
      }
      ...
      if (deviceId)
        this.deviceInflight.set(
          deviceId,
          this.deviceInflight.get(deviceId) - 1,
        );
    }
  }
  async pair(body) {
    ...
    try {
      ...
      requireThat(
        this.store.all("device").filter((d) => !d.revoked).length < 16,
        "DEVICE_LIMIT",
      );
  ...
  async read(d, method, p) {
    ...
    if (method === "session.list") {
      ...
      const rows = this.store
        ...
        .filter(
          (s) =>
            s.project === p.projectId &&
            (p.archived === undefined || s.archived === p.archived) &&
            (!p.query || s.title.toLowerCase().includes(p.query.toLowerCase())),
        )
        .sort(
          (a, b) =>
    ...
    if (method === "model.list") {
      fields(p, ["projectId", "cursor"], ["projectId"]);
      const project = this.project(d, p.projectId);
      if (p.cursor !== undefined) string(p.cursor, 4000);
      requireThat(this.adapter.models, "CAPABILITY_UNAVAILABLE");
      return this.adapter.models(project, p);
    }
    if (method === "operation.read") {
      fields(p, ["operationId"], ["operationId"]);
      identifier(p.operationId);
      return (
        this.store.get("operation", `${d.id}:${p.operationId}`) ?? {
          status: "not_found",
        }
      );
    }
    fields(
      ...
      method === "session.items"
        ? ["sessionId", "turnId", "cursor"]
        : ["session.read", "terminal.list"].includes(method)
          ? ["sessionId", "cursor"]
          : ["sessionId"],
      ["sessionId"],
    );
    ...
    if (method === "session.read") {
      ...
      if (s.nativePhase && s.nativePhase !== "ready")
        return {
          session: s,
          snapshot: { status: "needs-reconciliation" },
          cursor,
        };
    ...
    if (method === "session.items") {
      requireThat(this.adapter.items, "CAPABILITY_UNAVAILABLE");
      return this.adapter.items(s, p);
    }
    if (method === "terminal.list") {
      requireThat(this.adapter.terminals, "CAPABILITY_UNAVAILABLE");
      return this.adapter.terminals(s, p);
    }
  }
  async ensure(s, authorize = () => {}) {
    if (s.nativePhase && s.nativePhase !== "ready")
      requireThat(s.upstream, "NATIVE_SESSION_RECONCILIATION_REQUIRED");
    if (this.live.has(s.id) && (!s.nativePhase || s.nativePhase === "ready"))
      return;
    await this.adapter.resume(s, authorize);
    s.nativePhase = "ready";
  ...
  async mutateNow(d, method, p) {
    if (method === "session.create") {
      ...
      Object.assign(s, this.adapter.validateSettings?.(p.settings ?? {}) ?? {});
      this.current(d);
      s.nativePhase = "creating";
      this.store.put("session", s.id, s);
      const data = await this.adapter.create(s, project, () =>
        this.project(d, project.id),
      );
    ...
    if (method === "attachment.upload") {
      ...
      this.project(d, p.projectId);
      requireThat(
        ["text/plain", "image/png", "image/jpeg"].includes(p.mime),
        "ATTACHMENT_TYPE",
      );
      ...
      requireThat(
        this.store.all("attachment").filter((a) => a.device === d.id).length <
          20,
        "ATTACHMENT_QUOTA",
      );
      const a = {
    ...
    const keys = {
      ...
      "session.update": ["sessionId", "lease", "title", "settings"],
      "session.fork": ["sessionId", "lease", "title", "lastTurnId"],
      "session.compact": ["sessionId", "lease"],
      "terminal.stop": ["sessionId", "lease", "processId"],
      "lease.acquire": ["sessionId"],
      "lease.renew": ["sessionId", "lease"],
      "lease.release": ["sessionId", "lease"],
      "turn.start": ["sessionId", "lease", "text", "attachments", "settings"],
    ...
    if (method === "session.archive") {
      ...
      await this.adapter.archive?.(s, authorize);
    ...
    if (method === "session.update") {
      ...
      await this.adapter.quiescent?.(s);
      requireThat(this.adapter.update, "CAPABILITY_UNAVAILABLE");
      const settings = this.adapter.validateSettings?.(p.settings ?? {}) ?? {};
      if (p.title !== undefined) string(p.title, 500);
      Object.assign(
        s,
        await this.adapter.update(
          s,
          { ...settings, ...(p.title !== undefined ? { title: p.title } : {}) },
          authorize,
        ),
      );
      s.updatedAt = Date.now();
      this.store.put("session", s.id, s);
      this.emit(s.id, { type: "session.updated" });
      return { updated: true };
    }
    if (method === "session.fork") {
      ...
      const child = {
        ...s,
        id: randomUUID(),
        title: string(p.title ?? s.title + " (fork)", 500),
        archived: false,
        updatedAt: Date.now(),
      };
      delete child.upstream;
      delete child.nativeArchived;
      delete child.nativeArchivePending;
      child.nativePhase = "forking";
      this.store.put("session", child.id, child);
      Object.assign(child, await this.adapter.fork(s, child, p, authorize));
      child.nativePhase = "ready";
      this.store.put("session", child.id, child);
      this.live.add(child.id);
    ...
    if (method === "terminal.stop") {
      requireThat(this.adapter.stopTerminal, "CAPABILITY_UNAVAILABLE");
      string(p.processId, 500);
      const result = await this.adapter.stopTerminal(s, p.processId);
      try {
        await this.releaseProject(s);
      } catch (e) {
        if (
          !["TURN_NOT_QUIESCENT", "BACKGROUND_TERMINALS_ACTIVE"].includes(
            e.code,
          )
        )
          throw e;
    ...
    if (method === "session.compact") {
      requireThat(this.adapter.compact, "CAPABILITY_UNAVAILABLE");
      await this.adapter.quiescent?.(s);
      ...
      return this.adapter.compact(s, authorize);
    }
    string(p.text, 128000);
    if (method === "turn.steer") return this.adapter.steer(s, p.text);
    requireThat(
      !p.attachments ||
        (Array.isArray(p.attachments) && p.attachments.length <= 4),
      "ATTACHMENT_LIMIT",
    );
  ...
  ask(session, request, signal) {
    ...
    if (this.asks.size >= 32)
      return Promise.reject(new Fault("APPROVAL_LIMIT"));
    const id = randomUUID();
