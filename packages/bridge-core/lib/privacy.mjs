...
export async function privateDirectory(
  directory,
  { create = false, empty = false } = {},
) {
  if (create) await mkdir(directory, { recursive: true, mode: 0o700 });
  const info = await stat(directory);
  requireThat(info.isDirectory(), "PRIVATE_DIRECTORY_REQUIRED");
  if (empty)
    requireThat(
  ...
  if (process.platform !== "win32") {
    ...
    if (create) await chmod(directory, 0o700);
      ...
      requireThat((info.mode & 0o077) === 0, "PRIVATE_DIRECTORY_PERMISSIONS");
    return;
  }
