...
export async function privateDirectory(directory,{create=false,empty=false}={}) {
 ...
 const set=`$phase='SET';$acl=[Security.AccessControl.DirectorySecurity]::new();$acl.SetOwner($sid);$acl.SetAccessRuleProtection($true,$false);$rule=[Security.AccessControl.FileSystemAccessRule]::new($sid,[Security.AccessControl.FileSystemRights]::FullControl,([Security.AccessControl.InheritanceFlags]::ContainerInherit -bor [Security.AccessControl.InheritanceFlags]::ObjectInherit),[Security.AccessControl.PropagationFlags]::None,[Security.AccessControl.AccessControlType]::Allow);$acl.AddAccessRule($rule);Set-Acl -LiteralPath $p -AclObject $acl;`;
 const check=`$phase='CHECK';$acl=Get-Acl -LiteralPath $p;if(!$acl.AreAccessRulesProtected){throw 'ACL_INHERITANCE'};if(!$acl.GetOwner([Security.Principal.SecurityIdentifier]).Equals($sid)){throw 'ACL_OWNER'};$rules=$acl.GetAccessRules($true,$true,[Security.Principal.SecurityIdentifier]);if($rules.Count -ne 1 -or !$rules[0].IdentityReference.Equals($sid) -or $rules[0].AccessControlType -ne [Security.AccessControl.AccessControlType]::Allow -or $rules[0].FileSystemRights -ne [Security.AccessControl.FileSystemRights]::FullControl){throw 'ACL_MISMATCH'};`;
