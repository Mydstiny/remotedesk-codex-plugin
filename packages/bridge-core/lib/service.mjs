...
export async function service(action,options){
 ...
 const exists=async()=>{
  ...
  if(platform==='linux'){const value=await run('systemctl',['--user','show',def.id+'.service','--property=LoadState','--value']);return value.trim()!=='not-found';}
 ...
 if(action==='install'){
  ...
  try{const old=await readFile(def.path,platform==='win32'?'utf16le':'utf8');requireThat(old===def.text,'SERVICE_DEFINITION_CHANGED');}catch(e){if(e.code!=='ENOENT')throw e;await writeFile(def.path,def.text,{encoding:platform==='win32'?'utf16le':'utf8',mode:0o600,flag:'wx'});}
  ...
  if(!await exists()){
   ...
   if(platform==='win32')await run('schtasks.exe',['/Create','/TN',def.id,'/XML',def.path]);
  }
  if(platform==='linux')await run('systemctl',['--user','enable','--now',def.id+'.service']);
  if(platform==='win32')await run('schtasks.exe',['/Run','/TN',def.id]);
  return {installed:true,id:def.id};
 }
 requireThat(registered,'SERVICE_NOT_REGISTERED');
 if(action==='stop'||action==='uninstall'){await requestStop(options.state);if(action==='stop')return {action,id:def.id,stopped:true};}
 const present=await exists();let result;
 if(action!=='uninstall')requireThat(present,'SERVICE_NOT_REGISTERED_RETRY_INSTALL');
 if(present){
  if(platform==='darwin')result=await run('launchctl',action==='status'?['print',target]:action==='start'?['kickstart',target]:['bootout',`gui/${process.getuid()}`,def.path]);
  if(platform==='linux')result=await run('systemctl',['--user',...(action==='uninstall'?['disable','--now']:[action]),def.id+'.service']);
  if(platform==='win32')result=await run('schtasks.exe',action==='status'?['/Query','/TN',def.id,'/FO','LIST']:action==='start'?['/Run','/TN',def.id]:['/Delete','/TN',def.id,'/F']);
 }
 if(action==='uninstall'){await unlink(def.path).catch(e=>{if(e.code!=='ENOENT')throw e;});await unlink(record);if(platform==='linux')await run('systemctl',['--user','daemon-reload']);}
 return {action,id:def.id,...(action==='status'?{nativeStatus:result}:{})};
}

export async function requestStop(state){
 const lock=join(state,'server.lock'),request=join(state,'stop.request');let owner;
 try{owner=JSON.parse(await readFile(lock,'utf8'));}catch(e){if(e.code==='ENOENT')return;throw e;}
 requireThat(Number.isSafeInteger(owner.pid)&&owner.pid>0,'LOCK_PID_INVALID');
...
export function watchStopRequests(state){
 let requested=false,busy=false;const file=join(state,'stop.request');
 const timer=setInterval(async()=>{if(requested||busy)return;busy=true;try{const value=JSON.parse(await readFile(file,'utf8'));if(value.pid===process.pid&&process.listenerCount('SIGTERM')>0){requested=true;await unlink(file);process.emit('SIGTERM');}}catch{}finally{busy=false;}},300);timer.unref();return()=>clearInterval(timer);
}
