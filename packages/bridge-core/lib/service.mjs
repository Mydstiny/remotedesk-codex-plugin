...
export function serviceDefinition({
  ...
  requireThat(/^S-1-(?:[0-9]+-)*[0-9]+$/.test(userSid ?? ''), 'WINDOWS_USER_SID_REQUIRED');
...
export async function service(action, options) {
  ...
  requireThat(
    ['render', 'install', 'start', 'stop', 'status', 'uninstall'].includes(action),
    'SERVICE_ACTION_INVALID',
  );
  if (action === 'render') return def;
  const call = (command, args) =>
    exec(command, args, { timeout: 60000, maxBuffer: 32000, windowsHide: true });
  const run = async (command, args) => {
    try {
      return (await call(command, args)).stdout;
    } catch (e) {
      options.diagnostic?.({ command, action: args[0], code: e.code, stderr: e.stderr });
  ...
  const exists = async () => {
    ...
    requireThat(['EXISTS', 'ABSENT'].includes(value.trim()), 'SERVICE_MANAGER_UNAVAILABLE');
  ...
  if (registered)
    requireThat(
      registered.id === def.id &&
        ...
        (action !== 'install' ||
          (registered.entry === options.entry && registered.node === process.execPath)),
  ...
  if (action === 'install') {
    ...
    if (!registered) {
      ...
      await writeFile(record, JSON.stringify(registered), { mode: 0o600, flag: 'wx' });
    }
    ...
    try {
      const old = await readFile(def.path, platform === 'win32' ? 'utf16le' : 'utf8');
    ...
    if (!(await exists())) {
      ...
      if (platform === 'linux') {
        ...
        await run('systemctl', ['--user', 'enable', '--now', def.id + '.service']);
      }
      if (platform === 'win32')
