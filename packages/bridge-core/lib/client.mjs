...
export class Client {
  ...
  async write(method, params, { operationId, epoch }) {
    ...
    return (
      await this.request("/v1/rpc", { method, params, operationId, epoch })
    ).operation;
  }
  events({ cursor, runtime, signal, onEvent }) {
    return new Promise((resolve, reject) => {
      const req = https.get(
        new URL(
          `/v1/events?cursor=${cursor}&runtime=${encodeURIComponent(runtime)}`,
          this.url,
        ),
        { ...this.tls, signal },
        (res) => {
...
export async function pairClient(
  ...
  const result = await client.request("/v1/pair", {
    code: invite.code,
    csr,
    name,
  });
  requireThat(
    result.instance === invite.serverInstance,
...
export async function loadClient(directory) {
  ...
  const config = JSON.parse(
    await readFile(join(directory, "client.json"), "utf8"),
  );
  ...
  requireThat(
    handshake.instance === config.instance &&
      handshake.deviceId === config.deviceId,
    "SERVER_IDENTITY_MISMATCH",
  );
...
export async function submit(directory, method, params, { retryId } = {}) {
  ...
  try {
    ...
    try {
      const operation = await client.write(
        packet.method,
        packet.params,
        packet,
      );
