...
async function openssl(args) {
  await exec('openssl', args, { timeout: 15000, maxBuffer: 16384, windowsHide: true });
}
export async function initializePki(directory, hosts = ['localhost', '127.0.0.1']) {
  ...
  if (process.platform !== 'win32')
    for (const f of ['ca.key', 'server.key']) await chmod(join(directory, f), 0o600);
}
...
export async function issueClient(directory, csr) {
  ...
  try {
    ...
    return { cert, fingerprint: identity.fingerprint256, expires: Date.parse(identity.validTo) };
  } finally {
...
export async function renewServer(directory) {
  ...
  requireThat(
    certificate.checkPrivateKey(
      (await import('node:crypto')).createPrivateKey(await readFile(join(directory, 'server.key'))),
    ),
