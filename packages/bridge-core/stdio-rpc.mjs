...
export class StdioRpc extends EventEmitter {
  ...
  constructor(
    command,
    args = [],
    { cwd, timeoutMs = 15000, maxFrameBytes = 1048576, maxPending = 32, requestHandler, env } = {},
  ) {
    super();
    ...
    const payload = Buffer.from(
      JSON.stringify({ command, commandLine: [command, ...args].map(quoteWindows).join(' ') }),
    ).toString('base64');
    this.#child = spawn(
    ...
    this.#child.once('close', () => {
      ...
      if (!this.#groupExists())
        this.#finishClose(!this.#forcedPipeClose && (!this.#child.pid || this.#stdoutEnded));
    });
    ...
    if (typeof message.method === 'string') {
      if (hasId) {
        ...
        if (!this.#requestHandler) {
          ...
          if (this.#serverPending.has(message.id) || this.#serverPending.size >= this.#maxPending)
            ...
            .then(
              ...
              () => {
                if (!this.#closed)
                  this.#write({
                    id: message.id,
                    error: { code: -32601, message: 'Request denied or unavailable' },
                  });
              },
            )
