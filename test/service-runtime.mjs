...
const options={engine:'codex',entry,state,diagnostic:value=>console.error(JSON.stringify({fixtureServiceDiagnostic:value}))};let installed=false;
...
try{
 const def=serviceDefinition({...options,userSid:'S-1-5-21-123-456-789-1001'});assert.notEqual(def.id,serviceDefinition({...options,state:state+'2',userSid:'S-1-5-21-123-456-789-1001'}).id);
...
}catch(e){try{console.error('FIXTURE_SERVICE_LOG\n'+await readFile(join(state,'service.log'),'utf8'));}catch{}throw e;}finally{
