...
const root = await mkdtemp(join(tmpdir(), 'remotedesk-service-')),
  state = join(root, 'private state'),
  entry = join(root, 'fixture host.mjs');
const cli = new URL('../packages/bridge-core/lib/cli.mjs', import.meta.url).href,
  server = new URL('../packages/bridge-core/lib/server.mjs', import.meta.url).href;
...
const options = {
  ...
  entry,
  state,
  diagnostic: (value) => console.error(JSON.stringify({ fixtureServiceDiagnostic: value })),
};
let installed = false;
const wait = async (fn) => {
  ...
  throw new Error('SERVICE_FIXTURE_TIMEOUT');
};
const running = async () => {
  try {
    const value = JSON.parse(await readFile(join(state, 'server.lock'), 'utf8'));
...
try {
  const def = serviceDefinition({ ...options, userSid: 'S-1-5-21-123-456-789-1001' });
  assert.notEqual(
    def.id,
    serviceDefinition({ ...options, state: state + '2', userSid: 'S-1-5-21-123-456-789-1001' }).id,
  );
  installed = true;
  ...
  installed = false;
  await assert.rejects(access(def.path));
  await assert.rejects(access(join(state, 'service.json')));
  assert.equal((await configuration(state)).engine, 'codex');
  console.log(
    'PASS actual current-user service manager: unique identity, install/retry/status, graceful stop/start, uninstall and state preservation (' +
      process.platform +
      ')',
  );
} catch (e) {
  try {
    console.error('FIXTURE_SERVICE_LOG\n' + (await readFile(join(state, 'service.log'), 'utf8')));
  } catch {}
  throw e;
} finally {
  if (installed) {
    try {
      ...
      try {
        ...
        console.error('FIXTURE_SERVICE_CLEANUP_REQUIRES_INSPECTION');
      }
    }
  }
