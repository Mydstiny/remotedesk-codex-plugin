...
import {
  service,
  serviceDefinition,
} from "../packages/bridge-core/lib/service.mjs";
...
const cli = new URL("../packages/bridge-core/lib/cli.mjs", import.meta.url)
    .href,
  server = new URL("../packages/bridge-core/lib/server.mjs", import.meta.url)
    .href;
...
const options = {
  ...
  state,
  diagnostic: (value) =>
    console.error(JSON.stringify({ fixtureServiceDiagnostic: value })),
};
let installed = false;
const wait = async (fn) => {
...
const running = async () => {
  try {
    const value = JSON.parse(
      await readFile(join(state, "server.lock"), "utf8"),
    );
...
try {
  ...
  assert.equal(
    await running(),
    false,
    "stop must fence a spawned process before Bridge.start",
  );
  ...
  assert.equal(
    await running(),
    false,
    "immediate start/stop must not leave a delayed listener",
  );
  await service("start", options);
  await wait(running);
  await service("stop", options);
