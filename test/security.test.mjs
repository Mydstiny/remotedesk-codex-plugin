...
import {
  ...
  loadClient,
  Client,
} from "../packages/bridge-core/lib/client.mjs";
  ...
  async () => {
    ...
    try {
      ...
      await assert.rejects(
        new Client({ url: t.url, ca, servername: "unlisted.invalid" }).read(
          "project.list",
        ),
      );
      const id = (await t.call("session.create", { projectId: "p" })).result
        .sessionId;
      ...
      await assert.rejects(
        t.call("session.create", { projectId: "p" }),
        /EPOCH_EXPIRED_RECONCILE/,
      );
      db.close();
    } finally {
      await t.close();
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        (await t.call("session.archive", { sessionId: id, lease })).status,
        "succeeded",
      );
      assert.equal(
        (await t.call("session.resume", { sessionId: id, lease })).status,
        "succeeded",
      );
      let finish;
      ...
      const read = t.client
        .read("session.read", { sessionId: id })
        .catch((e) => e);
      await entered;
...
test("cross-plugin project writer lock excludes overlapping controllers", async () => {
  ...
  try {
    ...
    await mkdir(path);
    const project = {
      path: await import("node:fs/promises").then((fs) => fs.realpath(path)),
    };
...
test("service definitions quote paths and use current-user non-elevated managers", () => {
  ...
  assert.match(
    linux.text,
    /ExecStart="\/node dir\/node" "\/app folder\/cli.mjs"/,
  );
  ...
  async () => {
    ...
    try {
      for (const action of ["turn.cancel", "session.archive"]) {
        const id = (await t.call("session.create", { projectId: "p" })).result
            .sessionId,
          lease = (await t.call("lease.acquire", { sessionId: id })).result
            .lease;
        let release,
          enteredResolve,
          running = false;
