...
test(
  ...
  async () => {
    ...
    try {
      ...
      const noCert = new Client({
        url,
        ca: await readFile(join(state, "pki", "ca.pem")),
      });
      ...
      assert.deepEqual(await client.read("project.list"), [
        { id: "work", title: "work" },
      ]);
      assert.equal(
      ...
      const lease = (await call("lease.acquire", { sessionId: id })).result
        .lease;
      ...
      const viewerInvite = await invite(state, {
        projects: ["work"],
        role: "viewer",
      });
      ...
      const streaming = client
        ...
        .catch((e) => e);
      const request = adapter.core.ask(
        id,
        { type: "command" },
      ...
      const newLease = (await call("lease.acquire", { sessionId: id })).result
        .lease;
      assert.equal(
        (
          await call("approval.answer", {
      ...
      recover.recover();
      assert.equal(
        recover.get("operation", "fixture:" + unknownId).status,
        "unknown",
      );
      recover.close();
    } finally {
