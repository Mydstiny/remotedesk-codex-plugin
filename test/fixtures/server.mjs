...
createInterface({ input: process.stdin }).on('line', (line) => {
  ...
  if (mode === 'descendant') {
    ...
    child.stdout.once('data', () => {
      ...
      setTimeout(() => send({ id: request.id, result: [child.pid, inherited.pid] }), 100);
    });
    return;
  }
  ...
  if (mode === 'approval') {
    first = request;
    send({ id: 'approval1', method: 'item/commandExecution/requestApproval', params: {} });
    return;
  }
  if (mode === 'error')
    return send({ id: request.id, error: { message: 'SECRET_SENTINEL', code: 7 } });
  if (mode === 'reverse') {
