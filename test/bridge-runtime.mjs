...
const root = await realpath(await mkdtemp(join(tmpdir(), 'remotedesk-codex-wire-'))),
  ...
  if (next === 'exec')
    return {
      call: {
        ...
        arguments: { command: "printf 'CODEX_DOCKER_OK' > codex-result.txt; cat codex-result.txt" },
      },
    };
  if (next === 'question')
...
try {
  ...
  bridge = new Bridge(state, new CodexAdapter({ providerOverrides: provider.overrides }));
  ...
  assert.equal(await readFile(join(workspace, 'codex-result.txt'), 'utf8'), 'CODEX_DOCKER_OK');
  await turn('exec', { decision: 'decline' });
  await turn('question', { text: 'CODEX_WIRE_ANSWER' });
  assert.ok(
  ...
  await waitFor(() => !bridge.adapter.runs.has(id));
  assert.ok(provider.calls.at(-1).images > 0, 'image reaches actual App Server model request');
  mode = 'hold';
  ...
  assert.ok(
    JSON.stringify(await client.read('session.read', { sessionId: id })).includes('STEER_FIXTURE'),
  );
  mode = 'hang';
  ...
  await turn('exec', { decision: 'accept' });
  assert.ok(events.some((e) => JSON.stringify(e).includes('CODEX_WIRE_COMPLETE')));
  abort.abort();
  await stream;
  ...
  console.log(
    'PASS Codex AppServer + Docker + mTLS: approval, question, attachments, streams, dedup, cancel, archive, full process restart/resume and revoke',
  );
} finally {
  await bridge?.stop();
